...
import { KeyRound, Plus, Pencil, Trash2, Shield, Clock, ExternalLink } from 'lucide-react';
...
import type { IRegistryCredential } from '@registry-vault/shared';
  ...
  function handleSubmit() {
    if (editingCredential) {
      ...
      createMutation.mutate(
        { registryConnectionId: formConnectionId, authType: 1, username: formUsername, password: formPassword },
        { onSuccess: () => setDialogOpen(false) },
      );
    }
  }
        ...
        <DialogContent className="sm:max-w-md">
          ...
          <div className="space-y-4 py-2">
            <div className="space-y-2">
              ...
              {!editingCredential && !connections?.length ? (
                <div className="flex items-center gap-2 rounded-md border border-dashed p-3 text-sm text-muted-foreground">
                  <ExternalLink className="h-4 w-4 shrink-0" />
                  No registry connections found. Add one in the <strong className="text-foreground mx-1">Registries</strong> tab first.
                </div>
              ) : (
                <>
                  <Select
                        ...
                        <SelectItem key={conn.id} value={conn.id}>
                          <div className="flex items-center gap-2">
                            <span className="text-[10px] font-mono bg-muted px-1.5 py-0.5 rounded">
                              {REGISTRY_TYPE_LABELS[conn.registryType]}
                            </span>
                            <span>{conn.name}</span>
                            <span className="text-muted-foreground text-xs truncate max-w-32">{conn.url}</span>
                          </div>
                        </SelectItem>
                      ))}
                    </SelectContent>
                  ...
                  {!editingCredential && (
                    <p className="text-xs text-muted-foreground">
                      Don't see your registry? Add it in the <strong>Registries</strong> tab first, including any local or private Docker/NPM/NuGet registries.
                    </p>
                  )}
                </>
              )}
