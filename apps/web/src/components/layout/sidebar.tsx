...
import { useRegistryConnections, useGeneralSettings } from '@/services/queries/settings.queries';
import { RegistryType, REGISTRY_LABELS } from '@registry-vault/shared';
...
const registryTypeConfig: Record<RegistryType, { icon: ReactNode; color: string; path: string }> = {
  [RegistryType.Docker]: {
...
function RegistryTypeGroup({
  ...
  const isTypePage = location.pathname === config.path;
    ...
    <Collapsible open={isOpen} onOpenChange={setIsOpen}>
      <div className="flex items-center gap-1 rounded-md px-1 py-0.5">
        <Link
          to={config.path}
          className={cn(
            'flex flex-1 items-center gap-2 rounded-md px-2 py-1.5 text-xs font-semibold transition-colors',
            isTypePage
              ? 'bg-sidebar-accent text-sidebar-accent-foreground'
        ...
        >
          <span className={config.color}>{config.icon}</span>
          <span>{label} ({connections.length})</span>
        </Link>
        <CollapsibleTrigger className="flex h-6 w-6 items-center justify-center rounded hover:bg-sidebar-accent/50 transition-colors">
          <ChevronDown
            className={cn(
...
function SidebarContent({ isCollapsed }: { isCollapsed: boolean }) {
  ...
  const instanceName = generalSettings?.instanceName || 'Registry Vault';
